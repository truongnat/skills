...
function linkRules(vendor, project) {
...
function saveInstallManifest(projectDir, manifest) {
    writeFileSync(join(projectDir, '.cursor', '.own-skills-install.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
}
...
function installCommandFiles(vendor, project, manifest) {
    const sources = [
...
async function main() {
    ...
    const cmd = argv._[0] === 'uninstall' ? 'uninstall' : 'install';
    ...
    if (cmd === 'install') {
        ...
        let mode = argv['skills-only'] ? 'skills' : 'full';

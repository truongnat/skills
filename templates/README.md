# Templates

Authoring and **output** templates for skills, workflows, prompts, and reports.

| Area | Path |
|------|------|
| Skill scaffold | [`skill/`](skill/) (see [`skills/examples/skill-template/`](../skills/examples/skill-template/)) |
| Workflow scaffold | [`workflow/w-template.md`](workflow/w-template.md) |
| Prompt scaffold | [`prompt/prompt-template.md`](prompt/prompt-template.md) |
| Report — code review | [`report/code-review.md`](report/code-review.md) |
| Report — security audit | [`report/security-audit.md`](report/security-audit.md) |
| Report — performance | [`report/performance-report.md`](report/performance-report.md) |
| Report — incident | [`report/incident-report.md`](report/incident-report.md) |
| Report — ADR | [`report/architecture-decision-record.md`](report/architecture-decision-record.md) |
| Issue — bug | [`issue/bug-report.md`](issue/bug-report.md) |
| Issue — feature | [`issue/feature-request.md`](issue/feature-request.md) |
| KB document | [`kb-document/knowledge-doc.md`](kb-document/knowledge-doc.md) |
| Prompt library (monolith) | [`PROMPT_TEMPLATES.md`](PROMPT_TEMPLATES.md) |

**Output style:** repo root [`OUTPUT_CONVENTIONS.md`](../OUTPUT_CONVENTIONS.md).

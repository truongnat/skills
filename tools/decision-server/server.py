...
class DecisionHandler(BaseHTTPRequestHandler):
    server_version = "SimpleSkillsDecisionServer/0.1"
    ...
    def _send(self, status: int, body: bytes, content_type: str) -> None:
        ...
        # Localhost-only tool; allow simple fetch from the served page.
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if target.suffix.lower() in {".html", ".htm"}:
            ...
            if CLIENT_JS_NAME not in text:
                inject = f'\n<script src="/{CLIENT_JS_NAME}" defer></script>\n'
                if "</body>" in text:
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/api/choice":
            ...
            # Also mirror into events for a single audit stream.
            append_jsonl(
...
if __name__ == "__main__":
    # Avoid inheriting broad proxy env into local tool usage unexpectedly.
